...
**SVG 须按 XML 语法书写，标签逐一闭合。** SVG 没有 HTML 那样的隐式闭合规则，漏写 `</rect>`、`</title>` 会改变元素嵌套关系。浏览器容错下画面常常看不出异常，所以这类错误只能靠检查发现。完整的生成硬规则与可填充的骨架模板见 `visual-guide.md`。**若用脚本批量修改 SVG，改完必须数一遍各标签的开闭数量**，正则重构时忘记写回闭合标签是已经发生过的事故。
